...
* **Field-level encryption**: By default, all document properties are encrypted. You must explicitly specify which properties to leave unencrypted via the `unencryptedProperties` field
...
* **Unencrypted Properties**: Only mark properties as unencrypted if they need to be indexed. Minimize the number of unencrypted properties.
